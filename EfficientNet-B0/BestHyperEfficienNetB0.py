# 5-fold cross-validation with efficientnetb0 and the best hyperparameters
...
optuna_pkl =r"C:/Users/Xuxu/Desktop/Master Thesis/OptunaEfficientNetB0Full/new_efficientnet_study.pkl"
...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_indices, labels[train_indices])):
        ...
        # class distribution per fold (sanity check)
        train_labels_fold = labels[train_indices[train_idx]]
        val_labels_fold = labels[train_indices[val_idx]]

        train_dist = np.bincount(train_labels_fold, minlength=len(class_to_idx))
        val_dist = np.bincount(val_labels_fold, minlength=len(class_to_idx))

        print(f"\n[fold {fold + 1}] class distribution check")
        print(f"{'class':>6} | {'train':>6} | {'val':>6} | {'ratio (val/train)':>16}")
        print("-" * 40)
        for cls_id in range(len(class_to_idx)):
            train_count = train_dist[cls_id]
            val_count = val_dist[cls_id]
            ratio = val_count / train_count if train_count > 0 else 0
            print(f"{cls_id:6d} | {train_count:6d} | {val_count:6d} | {ratio:16.4f}")
        ...
        train_loader = DataLoader(train_set, batch_size=32, shuffle=True, num_workers=2, pin_memory=True, persistent_workers=True, worker_init_fn=worker_init_fn)
        val_loader = DataLoader(val_set, batch_size=32, shuffle=False, num_workers=2, pin_memory=True, persistent_workers=True, worker_init_fn=worker_init_fn)

        model = EfficientNetB0Lightning(len(class_to_idx), class_weights, best_hparams)
